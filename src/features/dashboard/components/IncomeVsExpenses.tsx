...
export default function IncomeVsExpenses({ incomeCents, expensesCents, loading }: IncomeVsExpensesProps) {
  const total = incomeCents + expensesCents;
  const incomePct = total ? (incomeCents / total) * 100 : 0;
  const expensePct = total ? (expensesCents / total) * 100 : 0;
        ...
        <>
          <div className="ft-ive-row">
            ...
            <div className="ft-ive-value">
              {formatMoney(incomeCents, { maximumFractionDigits: 0 })}
            </div>
          </div>
          ...
          <div className="ft-ive-row" style={{ marginTop: 18 }}>
            ...
            <div className="ft-ive-value">
              {formatMoney(expensesCents, { maximumFractionDigits: 0 })}
            </div>
          </div>
          <div className="ft-ive-bar">
