...
import type { CashFlowPoint, DateRangeKey } from "@/types";
...
interface HoverState extends CashFlowPoint {
  x: number;
  y: number;
}

export default function CashFlowChart({ data, range, onRange, loading }: CashFlowChartProps) {
  ...
  const max = Math.max(1, ...data.map((d) => Math.max(d.incomeCents, d.expensesCents) / 100));
  ...
  const yTicks = 4;
  const ticks = useMemo(
    () => Array.from({ length: yTicks + 1 }, (_, i) => max * (i / yTicks)),
    [max],
  );
        ...
        {loading ? (
          ...
          >
              ...
              return (
                <g key={i}>
                  ...
                  <text x={PADDING_X - 8} y={y + 3} className="ft-chart-axis">
                    {formatMoney(v * 100, { maximumFractionDigits: 0 })}
                  </text>
                </g>
              );
            })}
            {data.map((d, i) => {
              ...
              const iH = (d.incomeCents / 100 / max) * innerH;
              const eH = (d.expensesCents / 100 / max) * innerH;
              const ix = cx - barW - 3;
              const ex = cx + 3;
              const delay = `${i * 30}ms`;
              return (
                <g
                  key={`${d.month}-${i}`}
                  onMouseEnter={() =>
                    setHover({ ...d, x: cx, y: PADDING_TOP + Math.min(iH, eH) })
                  }
            ...
            {hover && (
              <g className="ft-tooltip" style={{ pointerEvents: "none" }}>
                ...
                >
                  Income {formatMoney(hover.incomeCents, { maximumFractionDigits: 0 })}
                </text>
                <text
                  x={Math.min(WIDTH - 160, Math.max(16, hover.x - 70))}
                ...
                >
                  Expenses {formatMoney(hover.expensesCents, { maximumFractionDigits: 0 })}
                </text>
                <text
                  x={Math.min(WIDTH - 160, Math.max(16, hover.x - 70))}
                ...
                >
                  Net {formatMoney(hover.netCents, { maximumFractionDigits: 0 })}
                </text>
              </g>
            )}
          </svg>
